...
class Base(nn.Module):
    def __init__(self, dimension, latent_dimension, encoder=True, decoder=True, use_attention=True):
        super().__init__()
    ...
    def init_encoder(self):
        ...
        self.encoder = autoencoder.Encoder(self.encoder_channels, self.dimension, self.latent_dimension, self.use_attention)
    ...
    def load(self, path, map_location=torch.device('cpu')):
        if self.encoder: self.load_encoder(self.get_path(path, 'encoder'), map_location)
        if self.decoder: self.load_decoder(self.get_path(path, 'decoder'), map_location)
class Master(Base):
