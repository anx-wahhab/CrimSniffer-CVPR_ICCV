...
class Generator(nn.Module):
    def __init__(self, dimension, spatial_channel):
        ...
        # Auxiliary branch and GFFM
        self.auxiliary_branch = nn.ModuleList()
        self.gffm = nn.ModuleList()
        for i in range(len(self.encoder_channels) - 1):
            self.auxiliary_branch.append(nn.Conv2d(self.encoder_channels[-1-i], self.decoder_channels[i], kernel_size=1))
            self.gffm.append(block.GatedFeatureFusionModule(self.decoder_channels[i], self.decoder_channels[i]))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.decoder)):
            if i < len(self.auxiliary_branch):
                aux_feature = self.auxiliary_branch[i](x)
                x = self.gffm[i](x, aux_feature)
            x = self.decoder[i](x)
